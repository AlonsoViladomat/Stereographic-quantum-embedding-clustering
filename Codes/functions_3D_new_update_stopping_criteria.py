...
def classical_k_mean(k,data,fixed_centroids,maxNumIter,r, dataLabels):
    ...
    while numIter < maxNumIter:
        ...
        for i in np.arange(len(centroids)):
            centroids[i,[0,1,2]] = r*(data[dataClusters == centroids[i,3]].sum(axis = 0))/(np.linalg.norm(data[dataClusters == centroids[i,3]].sum(axis = 0)))      
