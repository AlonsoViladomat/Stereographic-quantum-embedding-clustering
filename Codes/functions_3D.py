...
def initialize_centroids(k,data,method,centroids):
    ...
    if method == "maxEstimate":
        ...
        centroids[:,[0,1,2]] = np.array([[x,y,z] for x in np.linspace(-1.1,1.1,8) for y in np.linspace(-1.1,1.1,8) for z in np.linspace(-1.1,1.1,8)])
...
def plot_dataClusters(dataClusters, fig, ax, numIter, centroids, data):
    ...
    ax.clear()
    for i in centroids[:, 3]:
        ax.plot(data[dataClusters == i, 0], data[dataClusters == i, 1],data[dataClusters == i, 2], '.', label=i,
                markersize=4, color=[i / 64, 1 / i, ((64 * (i % 2)) + i * pow(-1, (i % 2))) / 64])
    for i in range(len(centroids)):
        j = centroids[i, 3]
        ax.plot(centroids[i, 0], centroids[i, 1],centroids[i, 2] ,'.', markersize=8, color=[
...
def classical_k_mean(k,data,fixed_centroids,maxNumIter,coordinateSystem,method = 'fixed'):
