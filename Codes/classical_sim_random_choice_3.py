...
#stores results in a file
out_folder = os.path.join(cwd+ "/results/classical_random_choice")
try:
    os.mkdir(out_folder)
except FileExistsError:
    pass
...
for num_points in num_point:
    ...
    for iterator1 in range(100):
        ...
        #appends results
        results.append([num_points,avgAccuracy1,avgAccuracy2,diff,avgNumIter])
        print(num_points,avgAccuracy1,avgAccuracy2,diff,avgNumIter)

results = np.asarray(results)


string = file.partition(".")[0]
output_file = string + "_classical_random_choice" 
