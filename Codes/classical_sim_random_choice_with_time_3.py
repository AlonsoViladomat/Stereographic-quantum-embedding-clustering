...
for num_points in num_point:
    ...
    for iterator1 in range(100):
        ...
        for iterator2 in range(100):
            ...
            dataClusters, centroids, numIter = classical_k_mean(64,dataIn,alphabet,50)

            end = timer() 
    
            accuracy = accuracy_score(sampleLabelsIn, dataClusters)
            #ccuracy2 = balanced_accuracy_score(sampleLabelsIn, dataClusters)
            t = end - start 
        ...
        #appends results
        results.append([num_points,avgAccuracy,avgNumIter,avgTime])
        print(num_points,avgAccuracy,avgNumIter,avgTime)

results = np.asarray(results)


string = file.partition(".")[0]
output_file = string + "_classical_random_choice_timing_sim" 
